...
| [`lifted_islands.md`](lifted_islands.md) | The per-game hot-path hook method (the dos_re island technique on Win16): PC-sample → live-trace the hot loop → lift as a Python island → A/B pixel-oracle gate. The worked example is `microman/hooks.py`. |
...
- [`ppython/run_status.md`](ppython/run_status.md) — the journal (newest on
